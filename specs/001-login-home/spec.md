...
- **會話過期**: 使用者長時間未使用 APP 導致會話過期時，系統自動導向登入頁面
- **重複登入**: 使用者已在其他裝置登入時，系統處理多裝置登入情境
...
- **FR-010**: 系統必須在使用者點擊登出時清除所有會話資料
- **FR-011**: 系統必須在登出後導向登入頁面
...
- **會話 (Session)**: 代表使用者的登入狀態，包含會話 token、建立時間、過期時間、最後活動時間
